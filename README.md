...
> 说明：`select` 模型中数据传输（RETR/STOR）期间会临时切回阻塞，是课程设计范围内的简化，因此高并发大文件传输时吞吐略低于线程模型——这正是性能实验里值得讨论的点。
...
脚本会依次以三种并发模型启动服务器，对同一文件做并发下载，输出耗时与吞吐量对比表，可直接用于课程设计报告的性能分析章节。
...
教学型项目，欢迎学习、改造与二次开发；如用于课程作业请遵守所在课程的学术诚信要求。
